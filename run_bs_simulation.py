...
try:
    from app.simulation.config import load_default_config, BankingSupervisionConfig
...
def main():
    print("Loading default banking supervision configuration...")
    try:
        bs_config = load_default_config(simulation_type="banking_supervision")
        if not isinstance(bs_config, BankingSupervisionConfig):
            print(f"Error: Expected BankingSupervisionConfig, but got {type(bs_config)}")
            sys.exit(1)
    ...
    # print(bs_config.to_json(indent=2)) # Uncomment to see the full config
    
    print("\\nRunning Banking Supervision simulation with default config (no database)...")
    try:
        # Instantiate the simulator
        simulator = BankingSupervisionSimulator(config=bs_config, db=None)
        
        # Run the simulation
        results = simulator.run_simulation()
        print("\\nSimulation completed successfully.")
        
        output_filename = "bs_results.json"
        print(f"Saving Banking Supervision simulation results to {output_filename}...")
